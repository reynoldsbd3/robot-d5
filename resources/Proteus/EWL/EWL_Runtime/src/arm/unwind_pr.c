...
static uint8_t next_unwind_byte(uwdata *u) __attribute__((inline));

static uint8_t next_unwind_byte(uwdata *u) {
  uint8_t ub;
                                              ...
                                              uint32_t idx)
{
...
#ifdef PR_DIAGNOSTICS
			printf("PR Cleanup Entry Return address = 0x%x, range[0x%x-0x%x]\n",rtn_addr,rangestartaddr, rangestartaddr + length);
#endif
  ...
  #ifdef PR_DIAGNOSTICS
			printf("PR Catch entry Return address = 0x%x, range[0x%x-0x%x]\n",rtn_addr,rangestartaddr, rangestartaddr + length);
#endif
          	if (rangestartaddr <= rtn_addr && rtn_addr < rangestartaddr + length) {
              /* In range */
