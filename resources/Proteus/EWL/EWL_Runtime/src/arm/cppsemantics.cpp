...
extern "C" {
  ...
  void __cxa_call_unexpected(void *); /* will be casted to UCB * */
...
// efficient.
#if __CWCC__
#  define ARM_EXCEPTIONS_BUFFER_NOT_REQUIRED() 1
#elif 0
...
void __cxa_call_unexpected(void *ex_obj)
{
  UCB *ucbp = reinterpret_cast<UCB *>(ex_obj);
...
extern void _Unwind_Resume(UCB *);
  ...
  __asm ("mov r5, r11");
  __asm ("push {r1-r5}");
  __asm ("bl __ARM_cxa_end_cleanup");    // returns UCB address in r0
  __asm ("pop {r1-r5}");
  __asm ("mov r8, r2");
  __asm ("mov r9, r3");
  __asm ("mov r10, r4");
  __asm ("mov r11, r5");
  __asm ("pop {r1-r7}");
  __asm ("bl _Unwind_Resume");           // won't return
#else
  __asm ("stmfd r13!, {r1-r12}");
  __asm ("bl __ARM_cxa_end_cleanup");    // returns UCB address in r0
  __asm ("ldmia r13!, {r1-r12}");
  __asm ("b _Unwind_Resume");            // won't return
#endif
}
