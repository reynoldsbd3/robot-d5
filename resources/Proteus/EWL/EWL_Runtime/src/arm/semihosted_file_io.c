...
int sys_tmpnam(register void * tnp)
{
		...
		// save return address on stack
		__asm ("str		lr,[sp,#-4]!");
		__asm ("mov		r1,%0"::"r"(tnp));
		__asm ("mov		r0,%0"::"i"(SYS_TMPNAM));
		__asm ("swi		%0"::"i"(SEMIHOSTING_SWI));
...
{
	open(":tt",O_WRONLY|O_CREAT|O_TRUNC); // stdout
	open(":tt",O_RDONLY); // stdin
}
